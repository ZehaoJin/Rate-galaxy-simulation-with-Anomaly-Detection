...
class BaseModel():
    ...
    def save_weights(self, epoch):
        ...
        torch.save({'epoch': epoch + 1, 'state_dict': self.netg.state_dict()},
                   '%s/netG_%s.pth' % (weight_dir,int(epoch)))
        torch.save({'epoch': epoch + 1, 'state_dict': self.netd.state_dict()},
                   '%s/netD_%s.pth' % (weight_dir,int(epoch)))
    ...
    def train_one_epoch(self):
        ...
        # It prints the "current error", the error of last image in this epoch. It's kind of informative, kind of not.
    ...
    ## my version since I don't care about the AUC score
    def train(self):
        """ Train the model
        """
